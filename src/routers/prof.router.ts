...
export class ProfRouter {
  ...
  constructor(private router: Router, profController: ProfController) {
    ...
    // prof routes
    this.router.post("/get-profs", this.getProfs);
    ...
    this.router.post('/put-gpa',JWT.ProfAuth , this.addCourse)
    this.router.post("/student-course", JWT.ProfAuth, this.getStudentCoruse);
  }
  ...
  getStudentCoruse = async (request:Request ,response:Response) => {
    try {
      response.send('addCourse')
    } catch (error) {
